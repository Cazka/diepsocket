 ...
 *  - options {Object}
 * 		- timeout: How long the connection is allowed to take to establish. Default 30 seconds.
 *
 * Event: 'open'
...
class DiepSocket extends EventEmitter {
	...
	_onOpen(party) {
		...
		this._acceptTimeout = setTimeout(() => {
			if (!this._accepted) {
				this._emitTimeout(new Error("Timeout: Socket openend, but didn't receive 07 paket"));
				this.close();
			}
		}, 10000);
		super.emit('open');
	...
	_onMessage(data) {
		switch (data[0]) {
			...
			case 7:
				...
				break;
			case 9:
				this._socketOnError(new Error('Link is invalid or the server is getting botted'));
				break;
			default:
